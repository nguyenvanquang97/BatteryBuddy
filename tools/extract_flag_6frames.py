...
# Benchmark scale calibrated against pet_look_front_01.png (288px standing height)
# In source 3x2, cat body height is ~308px -> scale = 288.0 / 308.0 = 0.9350
SCALE = 0.9300
...
def isolate_main_component(crop_rgba, cw, ch):
    ...
    # Largest connected component is the cat and flag
    main_comp = max(components, key=len)
    keep_set = set(main_comp)

    # Clean other small stray artifacts
...
def main():
    src_path = "artifacts/media_flag_3x2.png"
    w, h, rgba = read_png_rgba(src_path)

    # 6 cells defined with safe overlap margin
    cells = [
        ("pet_flag_01", 0, 150, 350, 490),
        ("pet_flag_02", 330, 90, 700, 490),
        ("pet_flag_03", 670, 15, 1024, 490),
        ("pet_flag_04", 0, 560, 355, 900),
        ("pet_flag_05", 330, 560, 700, 900),
        ("pet_flag_06", 670, 560, 1024, 900)
    ]

    print(f"Extracting 6 frames with calibrated scale S={SCALE:.4f} against pet_look_front_01.png...")
    ...
    for name, x0, y0, x1, y1 in cells:
        ...
        # Place onto 384x320 canvas with ground baseline at TARGET_GROUND_Y
