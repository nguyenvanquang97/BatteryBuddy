#!/usr/bin/env python3
"""Extract the 2x4 poke-jump and angry-look animation sheet."""
...
ROWS = 2
COLUMNS = 4
ROW_PREFIXES = ("pet_poke_jump", "pet_angry_look")


def main() -> None:
    project_root = Path(__file__).resolve().parents[1]
    source_path = project_root / "artwork/pet_poke_reaction_source.png"
    output_dir = project_root / "app/src/main/res/drawable-nodpi"
    contact_sheet_path = project_root / "artifacts/pet-poke-reaction-contact-sheet.png"
    ...
    for row, prefix in enumerate(ROW_PREFIXES):
        ...
        for column in range(COLUMNS):
            ...
            isolated = isolate_meaningful_components(frame, max_components=8)
    ...
    print(f"Exported {len(outputs)} poke reaction frames to {output_dir}")
